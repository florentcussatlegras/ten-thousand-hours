...
export default function MasteryClient({
  ...
  const totalHoursAll =
    mastered.reduce((acc, t) => acc + t.totalSeconds, 0) +
    inProgress.reduce((acc, t) => acc + t.totalSeconds, 0);
  const avgTimeToMaster = mastered.length
    ? Math.round(
        mastered.reduce((acc, t) => acc + t.totalSeconds, 0) /
          mastered.length /
          3600,
              ...
              <div className="p-4 rounded-lg border border-gray-200">
                ...
                <div className="mt-3 space-y-3">
                  ...
                  {inProgress.map((t) => {
                    const pct = Math.min(
                      100,
                      Math.round((t.totalSeconds / (10000 * 3600)) * 10000) /
                        100,
                    );
                    return (
                      <div key={t.id}>
                        <div className="flex justify-between text-sm">
                          ...
                          <div className="text-default-500">
                            {Math.round(t.totalSeconds / 3600)}h
                          </div>
                        </div>
                  ...
                  <Link
                    href="/studies"
                    className="text-sm px-3 py-1 bg-sky-500 text-white rounded-md"
                  >
                    Voir tous mes apprentissages
                  </Link>
                </div>
              </div>
            </aside>
                    ...
                    <div className="flex justify-between items-start gap-3">
                      <div>
                        ...
                        <div className="text-xs text-default-500 mt-1">
                          Atteint le {formatDate(t.reachedAt)}
                        </div>
                      </div>
                ...
                <ol className="relative border-l border-gray-200">
                  {mastered.map((t, i) => (
                    <li key={t.id} className="mb-6 ml-6">
                      ...
                      <div className="text-xs text-default-500">
                        {formatDate(t.reachedAt)} — {hours(t.totalSeconds)} h
                      </div>
                    </li>
                  ))}
                </ol>
